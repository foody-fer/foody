...
function Login({ disabled }: { disabled: boolean }) {
    return (
        <>
            ...
            <button onClick={() => window.location.href = `${api}/oauth2/authorization/google`} 
                ...
                ${disabled ? "" : "cursor-pointer duration-200 hover:bg-[#a0b87e] hover:shadow-2xl  hover:scale-105"}`} disabled={disabled} role="button">
            ...
            <button onClick={() => window.location.href = `${api}/oauth2/authorization/github`}
            ...
            <div className="flex ml-1.5 mt-1">
                <div className="w-[4.5rem] sm:w-[6rem] md:w-[8.5rem] bg-textColor h-[1px] mt-[0.8rem]">
                </div>
